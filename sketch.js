...
// 음표 모양 위치 (왼쪽/오른쪽)
const noteShapeLeft = [
  { x: 0.1, y: 0.5 },   // 음표 머리 (왼쪽 하단)
  { x: 0.12, y: 0.3 },  // 음표 기둥 중간
  { x: 0.14, y: 0.15 }, // 음표 기둥 상단
];

const noteShapeRight = [
  { x: 0.9, y: 0.5 },   // 음표 머리 (오른쪽 하단)
  { x: 0.88, y: 0.3 },  // 음표 기둥 중간
  { x: 0.86, y: 0.15 }, // 음표 기둥 상단
];
...
class ColorParticle {
  constructor(colorData, intensity, side) {
    ...
    // 양 옆에서만 생성
    if (side === 'left') {
      this.x = random(0, width * 0.2); // 왼쪽 20% 영역
    } else {
      this.x = random(width * 0.8, width); // 오른쪽 20% 영역
    }
    
    this.y = random(-100, -20);
    this.speed = map(intensity, 0, 1, 4, 12);
  ...
  update() {
    ...
    if (!this.bouncing) {
      ...
      if (this.y >= height - 50) {
        this.bouncing = true;
        this.bounceY = height - 50;
        this.bounceVelocity = -6;
      }
    } else {
      // 바운스 물리
      this.bounceVelocity += 0.6;
      this.bounceY += this.bounceVelocity;
      this.y = this.bounceY;
      
      // 바운스 후 음표 모양으로 이동
      if (this.bounceVelocity > 0 && this.y >= height - 60) {
        this.settleIntoNote();
      }
    }
  }
  ...
  settleIntoNote() {
    ...
    // 랜덤 오프셋으로 자연스러운 쌓임
    this.targetX = targetPos.x * width + random(-30, 30);
    this.targetY = targetPos.y * height + random(-20, 20);
...
function analyzeAndPlayHarmony() {
  ...
  for (let i = 0; i < soundPlayers.length; i++) {
    ...
    if (ratio > 0.02) {
      let particleCount = floor(map(ratio, 0.02, 0.35, 0.6, 2.4, true));
      for (let p = 0; p < particleCount; p++) {
        if (random() < 0.36) {
          let intensity = map(ratio, 0.02, 0.35, 0.3, 1, true);
          let side = random() < 0.5 ? 'left' : 'right'; // 양쪽에서 랜덤 생성
...
function updateParticles() {
  ...
  for (let i = 0; i < accumulatedParticles.length; i++) {
    accumulatedParticles[i].display();
  }
